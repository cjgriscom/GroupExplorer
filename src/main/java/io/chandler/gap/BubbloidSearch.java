...
public class BubbloidSearch {
	// Bubbloid puzzles have dimensions like 2x2x3, 3x3x2

	// They are vertex-turning

	// On a 1x1x1 there are 12 edges.  Each edge has an orientation.


	// So let's define the vertices which operate upon the edges in a certain direction
	// Each edge has a left side toward one vertex and a right side toward another vertex
	...
	public static void main(String[] args) throws Exception {
		...
		for (int[] groupA : Permu.generateCombinations(4, axesInGroupA)) {
			for (int[] groupB : Permu.generateCombinations(4, axesInGroupB)) {
				...
				for (int bitmaskA = 0; bitmaskA < (1 << (axesInGroupA-1)); bitmaskA++) {
					for (int bitmaskB = 0; bitmaskB < (1 << (axesInGroupB-1)); bitmaskB++) {
						...
						for (int a = 0; a < groupA.length; a++) {
							int axisIndex = axisChoicesA[groupA[a]]-1;
							for (int b = 0; b < maxVertexDepth; b++) {
								generator[0][ai] = axes[axisIndex][b].clone();
						...
						for (int b = 0; b < groupB.length; b++) {
							int axisIndex = axisChoicesB[groupB[b]]-1;
							for (int c = 0; c < maxVertexDepth; c++) {
								generator[1][bi] = axes[axisIndex][c].clone();
								// Reverse if bitmask is set
								if ((bitmaskB & (1 << b)) != 0) {
