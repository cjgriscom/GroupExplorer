...
#define BITSET_WORDS 5  // 5 * 64 = 320 bits
...
#define knoten 320
#define d_kanten 2048  // Adjust if necessary for larger graphs
typedef int16_t GRAPH[knoten + 1][knoten];
typedef int16_t ADJAZENZ[knoten + 1];

// LONGTYPE original[knoten+1]; // the graph as bitvector

typedef struct K {
  uint16_t ursprung; /* bei welchem knoten startet die kante */
  uint16_t name;     /* Identifikation des Knotens, mit
...
static int16_t bfsnummer[knoten + 1], invbfsnummer[knoten + 1];
int good_approx[100] = {0};

// New bitset manipulation functions for Longtype
static inline Longtype longtype_zero(void) {
    Longtype res;
...
static inline Longtype longtype_from_bit(int i) {
    Longtype res = longtype_zero();
    int word = (i - 1) / 64;
    int bit = (i - 1) % 64;
    if(word < BITSET_WORDS) {
        res.words[word] = 1ULL << bit;
    }
    return res;
}

static inline void longtype_set_bit(Longtype *a, int i) {
    int word = (i - 1) / 64;
    int bit = (i - 1) % 64;
    if(word < BITSET_WORDS)
        a->words[word] |= (1ULL << bit);
}

static inline void longtype_clear_bit(Longtype *a, int i) {
    int word = (i - 1) / 64;
    int bit = (i - 1) % 64;
    if(word < BITSET_WORDS)
        a->words[word] &= ~(1ULL << bit);
...
static inline int longtype_is_set(const Longtype *a, int i) {
    int word = (i - 1) / 64;
    int bit = (i - 1) % 64;
    if(word < BITSET_WORDS)
        return (a->words[word] & (1ULL << bit)) != 0;
    return 0;
}

static inline Longtype longtype_or(Longtype a, Longtype b) {
    Longtype res;
    for (int i = 0; i < BITSET_WORDS; i++) {
...
static inline void longtype_or_assign(Longtype *dest, const Longtype *src) {
    for (int i = 0; i < BITSET_WORDS; i++) {
        dest->words[i] |= src->words[i];
    }
}

static inline Longtype longtype_and(Longtype a, Longtype b) {
    Longtype res;
    for (int i = 0; i < BITSET_WORDS; i++) {
        res.words[i] = a.words[i] & b.words[i];
    }
...
static inline Longtype longtype_not(Longtype a) {
    Longtype res;
    for (int i = 0; i < BITSET_WORDS; i++) {
...
static inline int longtype_popcount(const Longtype *a) {
    int count = 0;
    for (int i = 0; i < BITSET_WORDS; i++) {
        count += __builtin_popcountll(a->words[i]);
    }
    return count;
}

static inline int longtype_nonzero(Longtype a) {
    for (int i = 0; i < BITSET_WORDS; i++) {
        if (a.words[i] != 0) return 1;
    }
    return 0;
}

static inline int longtype_atleast_two_bits_expr(Longtype a) {
    return (longtype_popcount(&a) >= 2);
}
...
void writeset(char name[], Longtype s) {
  ...
  for (i = 1; i <= knoten; i++) {
    if (longtype_is_set(&s, i)) fprintf(stderr, " %d", i);
  }
  ...
  for (i = 1; i <= maxtop; i++)
    if (firstedge[i] != NULL) {
      ...
      do {
        ...
        do {
          longtype_set_bit(&localface, run2->ursprung);
...
void decodiere(uint16_t *code, GRAPH graph, ADJAZENZ adj, int codelaenge)
...
int lese_multicode(uint16_t **code, int *codelaenge, FILE *fil)
...
{
  static int16_t maxknotenzahl = -1;
  int16_t codel, gepuffert = 0;
  int16_t vertexnumber, nuller = 0;
  int byte1, byte2;

  // Read two bytes to get the big-endian short
  if ((byte1 = getc(fil)) == EOF) return EOF;
  if ((byte2 = getc(fil)) == EOF) return EOF;
  vertexnumber = (byte1 << 8) | (byte2 & 0xFF);

  if (vertexnumber <= 0) {
    // If 0 or negative is encountered, handle error or exit
    fprintf(stderr, "Found invalid vertexnumber=%d in lese_multicode\n", vertexnumber);
    return EOF;
  }

  if (vertexnumber > maxknotenzahl) {
    if (*code) free(*code);
    // Allocate enough memory for up to (vertexnumber*(vertexnumber-1)/2 + vertexnumber) short entries
    *code = (int16_t *)malloc((vertexnumber * (vertexnumber - 1) / 2 + vertexnumber) * sizeof(int16_t));
    if (*code == NULL) {
  ...
  // Store the number of vertices in the first slot
  (*code)[0] = vertexnumber;
  codel = 1;
  nuller = 0;

  // After reading the "vertexnumber", read edges as big-endian shorts until we get (vertexnumber - 1) zeros
  while (nuller < (vertexnumber - 1)) {
    if ((byte1 = getc(fil)) == EOF) return EOF;
    if ((byte2 = getc(fil)) == EOF) return EOF;
    int16_t val = (byte1 << 8) | (byte2 & 0xFF);
...
// a face
{
  ...
  *face = longtype_zero();
  longtype_set_bit(face, e1->ursprung);
  longtype_set_bit(face, e1->name);
  for (run = right1->invers->next; run != e2; run = run->invers->next) {
    longtype_set_bit(face, run->ursprung);
  }
  newface = face_pointer[em_faces];
  em_faces++;
  e1->faceleft = newface;
  *newface = longtype_zero();
  longtype_set_bit(newface, e1->ursprung);
  longtype_set_bit(newface, e1->name);
  right2->faceleft = newface;
  for (run = right2->invers->next; run != e1; run = run->invers->next) {
    longtype_set_bit(newface, run->ursprung);
...
// given edges
{
  ...
  } else  // two faces become one
  {
    ...
    longtype_or_assign(face, right2->faceleft);
    em_faces--;
...
// path is constructed. returns the number of embedded eges
{
  ...
  *genus = 0;
  ...
  y = graph[x][0];
  faces[0] = longtype_zero();
  longtype_set_bit(&faces[0], y);
  longtype_set_bit(&faces[0], x);
  em_vertices = 2;
  em_faces = 1;
...
void remove_edge(KANTE *edge, int *genus) {
      ...
      invedge)  // degree 1 -- startvertex will be deleted after removal
  {
    ...
    firstedge[end] = NULL;
    // adj_embedded[top]--;
    longtype_clear_bit(edge->faceleft, end);
    ...
    } else  // 2 vlakken worden 1 vlak  -- het genus blijft
    {
      ...
      face = edge->faceleft;
      longtype_or_assign(face, invedge->faceleft);
  ...
  *ell = 0;
  while (lell) {
                                                      ...
                                                      // embedded
    {
      ...
      bit_edgelist[*ell] = longtype_zero();
      longtype_set_bit(&bit_edgelist[*ell], localedgelist[besti][0]);
      longtype_set_bit(&bit_edgelist[*ell], localedgelist[besti][1]);
      (*ell)++;
...
// without the common vertices
{
  ...
  // edge earlier
  for (; i < ell; i += 2)
    if ( longtype_nonzero(longtype_and(bit_edgelist[i], lastsub1)) &&
         longtype_nonzero(longtype_and(bit_edgelist[i], lastsub2)) &&
    ...
    {
      {
        ...
        for (j = 0; j < em_faces; j++) {
          if ( longtype_atleast_two_bits_expr(longtype_and(bit_edgelist[i], *face_pointer[j])) ) {
            found2 = 1;
            j = em_faces;
          }
        }
    ...
    do {
      if (run2->faceleft == run->faceleft) {
        ...
        rec_genus_max(
            ...
            longtype_and(*(edgelist->faceleft), longtype_not(*(edgelist->invers->faceleft))),
            longtype_and(*(edgelist->invers->faceleft), longtype_not(*(edgelist->faceleft))));
...
// without the common vertices
{
  ...
  if (ell < edgelimit)  // that is: "close" to the leaves
  {
    ...
    for (j = best = 0; j < em_faces; j++) {
      if ( longtype_atleast_two_bits_expr(longtype_and(bit_edgelist[0], *face_pointer[j])) ) {
        best++;
      }
    }
    if (!best) {
...
void sort(uint16_t list[], int length)
{
  int i;
  uint64_t masks[BITSET_WORDS];
  ...
  /* set the corresponding bit for each element in list */
  for(i = 0; i < length; i++) {
    int num = list[i];
    int wordIndex = (num - 1) / 64;
    int offset = (num - 1) % 64;
    masks[wordIndex] |= (1ULL << offset);
  }

  int pos = 0;
  /* extract sorted order from each word in order */
  for (int wordIndex = 0; wordIndex < BITSET_WORDS; wordIndex++) {
    while (masks[wordIndex]) {
      int bitIndex = FFSL(masks[wordIndex]); // returns 1-indexed position
      list[pos++] = wordIndex * 64 + bitIndex;
      masks[wordIndex] &= (masks[wordIndex] - 1);
    }
  }
}
